...
class MantenimientosTest {
    ...
    @Test
    void obtenerTodosPorFechas_ConIdEquipo_DeberiaRetornarMantenimientosDelEquipoEntreFechas() throws Exception {
        // Arrange
        Long idEquipo = 1L;
        Date desde = Date.from(LocalDate.of(2024, 1, 1).atStartOfDay(ZoneId.of("America/Montevideo")).toInstant());
        Date hasta = Date.from(LocalDate.of(2024, 1, 31).atStartOfDay(ZoneId.of("America/Montevideo")).toInstant());

        List<Mantenimiento> mantenimientosEsperados = Arrays.asList(mantenimientoMock);
        when(mantenimientoRepository.findByEquipo_IdAndFechaMantenimientoBetween(eq(idEquipo), any(Date.class), any(Date.class)))
                .thenReturn(mantenimientosEsperados);

        // Act
        List<Mantenimiento> resultado = mantenimientoService.ObtenerTodosPorFechas(idEquipo, desde, hasta);

        // Assert
        assertNotNull(resultado);
        assertEquals(1, resultado.size());
        verify(mantenimientoRepository, times(1))
                .findByEquipo_IdAndFechaMantenimientoBetween(eq(idEquipo), any(Date.class), any(Date.class));
        verify(mantenimientoRepository, never()).findByFechaMantenimientoBetween(any(Date.class), any(Date.class));
    }

    @Test
    void obtenerTodosPorFechas_SinIdEquipo_DeberiaRetornarTodosLosMantenimientosEntreFechas() throws Exception {
        // Arrange
        Long idEquipo = null;
        Date desde = Date.from(LocalDate.of(2024, 1, 1).atStartOfDay(ZoneId.of("America/Montevideo")).toInstant());
        Date hasta = Date.from(LocalDate.of(2024, 1, 31).atStartOfDay(ZoneId.of("America/Montevideo")).toInstant());
        ...
        List<Mantenimiento> mantenimientosEsperados = Arrays.asList(mantenimientoMock);
        when(mantenimientoRepository.findByFechaMantenimientoBetween(any(Date.class), any(Date.class)))
                .thenReturn(mantenimientosEsperados);
        ...
        assertEquals(1, resultado.size());
        verify(mantenimientoRepository, times(1))
                .findByFechaMantenimientoBetween(any(Date.class), any(Date.class));
        verify(mantenimientoRepository, never())
                .findByEquipo_IdAndFechaMantenimientoBetween(anyLong(), any(Date.class), any(Date.class));
    }

    @Test
    void obtenerTodosPorFechas_ConIdEquipoCero_DeberiaRetornarTodosLosMantenimientosEntreFechas() throws Exception {
        // Arrange
        Long idEquipo = 0L;
        Date desde = Date.from(LocalDate.of(2024, 1, 1).atStartOfDay(ZoneId.of("America/Montevideo")).toInstant());
        Date hasta = Date.from(LocalDate.of(2024, 1, 31).atStartOfDay(ZoneId.of("America/Montevideo")).toInstant());
        ...
        verify(mantenimientoRepository, times(1))
                .findByFechaMantenimientoBetween(any(Date.class), any(Date.class));
        //verify(mantenimientoRepository, never())
        //        .findByEquipo_IdAndFechaMantenimienteBetween(anyLong(), any(Date.class), any(Date.class));
    }
}